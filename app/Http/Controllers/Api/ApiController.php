...
class ApiController extends Controller
{
    ...
    public function businessData(Request $request)
    {
		$url =  url('/');
        $businessArray = [];
        $business = Business::OrderBy('id', 'desc')->paginate(10);
        if (!$business) {
        ...
        if (!empty($business)) {
            ...
            foreach ($business as $key => $value)
            {
                ...
                $businessArray[$key]['logo']=!empty($value->logo) ? $url.'/storage/card_logo/'.$value->logo : "custom/img/logo-placeholder-image-2.png";
